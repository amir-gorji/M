...
import { scanWorkspace, formatScanSummary } from '../analysis/WorkspaceScanner.js';
import { renderQuestions, parseAnswers } from '../questions/QuestionGenerator.js';
...
function resolveSessionId(
  ...
  for (const h of context.history) {
    if (h instanceof vscode.ChatResponseTurn && h.result?.metadata?.['kitsuneSessionId']) {
...
export async function kitsuneHandler(
  ...
  try {
    ...
    if (request.command === 'generate' || request.prompt.trim().toLowerCase() === '/generate') {
    ...
    switch (session.phase) {
      ...
      case 'ready-to-generate':
        ...
        if (request.prompt.toLowerCase().includes('generat') || request.prompt.trim() === '') {
          await handleGenerate(session, stream, token);
        } else {
          stream.markdown(
...
async function handleAnalyze(
  ...
  stream.markdown(
    ...
    `**Types detected**: ${analysis.migrationTypes.map(t =>
      t.split('-').map(w => w[0]!.toUpperCase() + w.slice(1)).join(' ')
    ).join(', ')}\n\n` +
    (analysis.importChanges.length > 0
      ? `**Import changes detected**:\n` +
        analysis.importChanges.slice(0, 5).map(ic =>
          `- \`${ic.oldImport}\` → \`${ic.newImport}\``
        ).join('\n') + '\n\n'
      : '') +
    '---\n\n' +
  ...
  if (vscode.workspace.workspaceFolders && analysis.searchPatterns.length > 0) {
    scanWorkspace(analysis.searchPatterns, token).then(scanResult => {
  ...
  if (batch) {
    stream.markdown(renderQuestions(batch.questions, batch.batchNumber, batch.totalBatches, analysis));
...
async function handleAnswers(
  ...
  const allBatches = (await import('../questions/QuestionCatalog.js')).batchQuestions(session.questions, batchSize);
  ...
  if (nextBatch) {
    stream.markdown(
      ...
      renderQuestions(nextBatch.questions, nextBatch.batchNumber, nextBatch.totalBatches, session.analysis)
    );
    advanceBatch(session);
  } else {
...
async function handleScan(
  ...
  if (session.analysis.searchPatterns.length === 0) {
    stream.markdown('_No search patterns were detected for this migration. The workspace scan is not applicable._');
    return;
  }
...
function formatPreferencesSummary(prefs: Partial<import('../types.js').UserMigrationPreferences>): string {
  const rows: string[] = [
    `| Setting | Value |`,
    `|---|---|`,
